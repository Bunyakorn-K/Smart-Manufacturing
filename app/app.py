...
def load_ai_models():
    ...
    if os.path.exists('predictive_maintenance_model.pkl') and os.path.exists('preprocessor.pkl'):
        models['predictive'] = joblib.load('predictive_maintenance_model.pkl')
...
def load_data():
    ...
    # 🌟 เพิ่มการสร้างตัวแปร Rolling Mean ให้เหมือนตอน Train 🌟
    rolling_cols = ['temperature', 'vibration', 'pressure']
    for col in rolling_cols:
        df[f'{col}_rolling_mean'] = df.groupby('machine_id')[col].transform(lambda x: x.rolling(window=3, min_periods=1).mean())
        
    # เติมค่า NaN ที่เกิดจากแถวแรกๆ ด้วยค่าเดิมของมันเองเพื่อป้องกัน Error
    df = df.bfill()
...
if not ai_models['predictive'] or not ai_models['preprocessor']:
    st.sidebar.warning("⚠️ ไม่พบไฟล์ 'predictive_maintenance_model.pkl' หรือ 'preprocessor.pkl'")
# สร้าง 3 Tabs
    ...
    for index, row in df_machine.iterrows():
        ...
        with ph_anomaly.container():
            ...
            if ai_models['anomaly']:
                # 🌟 อัปเดตรายชื่อให้ตรงกับ realistic_features 🌟
                features_anomaly = [
            ...
            # 🔴 โค้ดส่วนที่หายไป (สั่งให้แสดงข้อความและวาดกราฟ Plotly) 🔴
            if is_anomaly: 
                st.error("🚨 **DETECTED ANOMALY!** พบความผิดปกติในพฤติกรรมของเซ็นเซอร์")
            else: 
                st.success("✅ **NORMAL BEHAVIOR** พฤติกรรมเครื่องจักรปกติ")
                
            # วาดกราฟแสดงอุณหภูมิเทียบกับจุด Anomaly
        ...
        with ph_predictive.container():
            ...
            if ai_models['predictive'] and ai_models['preprocessor']:
                try:
                    # 🔴 เปลี่ยนจากการดรอปคอลัมน์ เป็นการระบุ 5 คอลัมน์เป๊ะๆ ตามตอน Train 🔴
                    features_pred = ['temperature', 'vibration', 'humidity', 'pressure', 'energy_consumption']
                    X_pred = current_row_df[features_pred]
